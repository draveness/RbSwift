...
extension Hash {
    ...
    ///   - closure: A closure returns true to keep the element in the receiver 
    ///              hash if duplicate happens.
    /// - Returns: A new hash containing the contents of both hash.
    func merge(_ otherHash: Hash<Key, Value>, keepFirst: ((Key, Value, Value) -> Bool)? = nil) -> Hash<Key, Value> {
